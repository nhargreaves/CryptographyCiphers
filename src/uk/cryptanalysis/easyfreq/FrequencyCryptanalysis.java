...
public class FrequencyCryptanalysis {
	...
	public int cryptanalysis() {
		...
		// text to be read is called ciphertext

		FrequencyTable cipherTable;

		FrequencyAnalyser cipherFreq = new FrequencyAnalyser();
		cipherFreq.setText(ciphertext);
		cipherTable = cipherFreq.analyse();
		cipherTable.print();
		...
		for (char alphabet = 'a'; alphabet <= 'z'; alphabet++) {
			if (cipherTable.getFrequency(alphabet) > highestQuantity) {
				highestQuantity = cipherTable.getFrequency(alphabet);
				estimateForE = alphabet;
			}
		}
		// do -'a' to get placement of letter
		this.key = (estimateForE - 'a') - ('e' - 'a');
	...
	public void decrypt() {
		...
		int position;
		this.plaintext = ""; // empty plaintext of null value
		// scan through ciphertext character by character
		for (int i = 0; i < ciphertext.length(); i++) {
			char currentChar = ciphertext.charAt(i);
			// if character is a letter, we translate it
			if (Util.isValidLetter(currentChar)) {
				int charPosition = Util.charToIndex(currentChar);
				int newPosition = ((charPosition - this.key) % 26);
				if (newPosition < 0) { // if negative turn to positive value due
										// to javas handling of mod
					newPosition = newPosition + 26;
				}
				this.plaintext = this.plaintext + Util.indexToChar(newPosition);
			} else { // if not, it is punctuation and we can simply add it as is
				this.plaintext = this.plaintext + currentChar;
			}
		}
