...
public class VigenereCryptanalysis {
	...
	public String cryptanalysis() {
		...
		String[] partitions = new String[ciphertext.length()]; // TODO will NOT
																// be this big
																// but whatevs
		Arrays.fill(partitions, "");
		int[] partitionCount = new int[partitions.length];
		Arrays.fill(partitionCount, 0);

		// this fills partition array with many partitions of 3 from the text
		for (int i = 0; i < ciphertext.length() - 2; i++) {
			char1 = Character.toString(ciphertext.charAt(i));
			char2 = Character.toString(ciphertext.charAt(i + 1));
			char3 = Character.toString(ciphertext.charAt(i + 2));
			currentPartition = char1 + char2 + char3;
			

			for (int j = 0; j <= partitions.length - 2; j++) {
				if (partitions[j].equals(currentPartition)) {
					partitionCount[j]++;
		...
		//get the most frequent partition
		int largestValue = 0;
		int valueLocation = 0;
		for (int i = 0; i < partitionCount.length; i++) {
			if (partitionCount[i] > largestValue) {
				valueLocation = i;
				largestValue = partitionCount[i];

			}
		}
		
		String partitionKey = partitions[valueLocation];
		ArrayList<Integer> matchDistance = new ArrayList<Integer>();
		
		// finds all distances between locations partition key is at
		int temp = 0;
		for (int i = partitions.length-2 ; i >= 0; i--) {
			if (partitions[i].equals(partitionKey)) {
				if (temp > 0) {
					matchDistance.add(temp - i);
					temp = i;
				} else {
					temp = i;
				}
		...
		int[] matchDistArray = new int[matchDistance.size()];
		for (int k = 0; k < matchDistArray.length; k++) {
			matchDistArray[k] = matchDistance.get(k);
		}
		int keyLength = HelperClass.gcdLoop(matchDistArray); //gets greatest common denominator of these to find the keylength

		String cipherSection = "";
		int location = 0;
		String keySt = "";
		
	     StringBuffer key = new StringBuffer();

	     //puts each character of string into their own string - every 2nd letter every 3rd letter etc
		for (int curSect = 0; curSect < keyLength; curSect++) { //go through sections 0 to key size
			while (location < ciphertext.length() - 20) { //go through ciphertext for each key size value
				for (int i = 0; i < keyLength; i++) { //count from 0 to keylength
					if (i == curSect) { //if i is eg consistantly 2nd letter put it into a little string
						cipherSection = cipherSection + Character.toString(ciphertext.charAt(location));
					}
					if (i == keyLength) { //if reached keylength reset to 0 to continue
						i = 0;
					}
			...
			location = 0; //set location back to 0 so it goes back through ciphertext on next key 
			keySt = keySt + (helperClass.getKeyValue(cipherSection)); //analyse through frequencyanalysis to find character used to transcribe this
			
			cipherSection = ""; //empty for next
		}
		key.append(keySt);
		this.key = key;
